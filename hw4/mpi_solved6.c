...
if (rank < 2) {
  nreqs = REPS*2;
  offset=0;         //offsets not working here.
  if (rank == 0) {
    src = 1;
    //offset = 0;
    }
...
// This comment above shows the error: task 2 hits this Waitall, but it's
// issued a blocking (not non-blocking) operation. Therefore, we need to
// except it from this call:
if (rank !=2)
