...
// Need a float to pass from Isend to Irecv
float gamma;
...
if (rank == 0) {
  ...
  for (i=0; i<10; i++) {
    ...
    // We're going to send a float so we can receive a float, so cast it
    gamma = (float) alpha;

    // change MPI_INT to MPI_FLOAT
    MPI_Isend(&gamma, 1, MPI_FLOAT, 1, tag, MPI_COMM_WORLD, &reqs[i]);
