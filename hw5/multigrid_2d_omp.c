...
double compute_norm(double **u, int N){
    int i, j;
    double norm = 0.0;
#pragma omp parallel if(N>16) shared(u, N) private(i, j)
    {
...
void coarsen(double **uf, double **uc, int N) {
    int ic, jc;
#pragma omp parallel if(N>16) shared(uc, uf, N) private(ic, jc)
    {
#pragma omp for collapse(2)
    for (ic = 1; ic < N/2; ++ic)
        for (jc = 1; jc < N/2; ++jc){
...
void refine_and_add(double **u, double **uf, int N)
{
    int i,j;
    //uf[1] += 0.5 * (u[0] + u[1]);
#pragma omp parallel if(N>16) shared(u, uf, N) private(i, j)
    {
#pragma omp for collapse(2)
    for (i = 1; i < N; ++i) {
        for (j = 1; j < N; ++j) {
...
{
    int i,j;
#pragma omp parallel if(N>16) shared(res, rhs, N, invhsq, u) private(i, j)
    {
#pragma omp for collapse(2)
    for (i = 1; i < N; i++)
        for (j = 1; j < N; j++)
...
void jacobi(double **u, double **rhs, int N, double hsq, int ssteps)
{
    ...
    for (s = 0; s < ssteps; ++s) {
#pragma omp parallel if(N>16) shared(rhs, N, hsq, u, unew, omega) private(i, j)
    {
#pragma omp for collapse(2)
        for (i = 1; i < N; i++){
            for (j = 1; j < N; j++){
...
{
    int i, j, Nfine, l, iter, max_iters, levels, numthreads, ssteps = 3;

    if (argc < 3 || argc > 4) {
        fprintf(stderr, "Usage: ./multigrid_2d_omp Nfine maxiter numthreads [s-steps]\n");
        fprintf(stderr, "Nfine: # of intervals, must be power of two number\n");
        fprintf(stderr, "numthreads: # of OMP threads\n"); 
